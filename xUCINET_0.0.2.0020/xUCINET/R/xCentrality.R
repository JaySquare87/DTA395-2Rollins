...
#' @param Diagonal Logical statement, whether diagonal should be included.
#'
#' @return A matrix with degree and normalized degree.
#' @export
#'
#' @examples
#' ## Directed valued one-mode with missing data and with diagonal
#' M1<-matrix(c(6,2,4,6,1,NA,2,NA,3),3,3, byrow=TRUE)
#' xDegreeCentrality(M1, Diagonal=FALSE)
#'
#' ## To obtain the indegree, use the transposed t()
#' xDegreeCentrality(t(M1), Diagonal=FALSE)
#'
...
#'
xDegreeCentrality<-function(NET1, Diagonal=FALSE)
{
  NN1<-dim(NET1)[1]
  TwoMode1<-dim(NET1)[1]!=dim(NET1)[2]
  if (TwoMode1==TRUE) {cat("TwoMode network\n")}
  if (Diagonal==FALSE & TwoMode1==FALSE) {diag(NET1)<-NA}
...
#'
#' @param NET1 A one-mode or two-mode network stored as a 'matrix' object.
#' @param Diagonal Logical statement, whether diagonal should be used.
#'
...
xEigenvectorCentrality<-function(NET1, Diagonal=FALSE)
{
  NN1<-dim(NET1)[1]
  if (Diagonal==FALSE) {diag(NET1)<-0}
  OUTPUT1<-matrix(NA,NN1,2)
...
#'
xBetaCentrality<-function(NET1, Beta=NA, Diagonal=FALSE, Option="None")
{
  #eigen1<-max(eigen(NET1)$values)
  eigen1 <- eigen(NET1,only.values=TRUE)$values[1]
  eigen1 <- Re(eigen1)
  if (is.na(Beta)) {Beta = 0.9999/eigen1}
...
#' No missing data are allowed. Diagonal for the geodesic distance is set to 0.
#' - Last updated: 1 February 2022.
#'
#' @param NET1 A one-mode or two-mode network stored as a 'matrix' object.
#'
...
#' The diagonal for the geodesic distance matrix is by default set to NA.
#' Missing data should be indicated by 'NA'.
#' - Last updated: 1 February 2022.
#'
#' @param NET1 A one-mode or two-mode network stored as a 'matrix' object.
#' @param kReach A single value or vector contain kReach values.
#' @param Diagonal Logical statement, whether diagonal should be included (default=FALSE).
...
#'
xReachCentrality<-function(NET1, kReach=2, Diagonal=FALSE, Normalize=FALSE, Cumulative=TRUE)
{
  ...
  if (Diagonal==FALSE) {diag(GDIST)<-NA}
  kn<-1
...
#' Beta is expected to be between 0 and 1
#' The diagonal for the geodesic distance matrix is by default set to NA.
#' Missing data should be indicated by 'NA'.
#' - Last updated: 17 Sep 2022.
#'
#' @param NET1 A one-mode or two-mode network stored as a 'matrix' object.
#' @param Beta A single valsue or vector contain Beta values.
#' @param Diagonal Logical statement, whether diagonal should be included (default=FALSE).
#' @param Normalize Whether to return the normalized value (divide by theoretical maximum value) or raw counts.
#'
#' @return A matrix with Beta reach centrality values for each value of beta
#' @importFrom sna geodist
#' @export
#'
#' @examples
...
#'
xBetaReachCentrality<-function(NET1, Beta=.5, Diagonal=FALSE, Normalize=FALSE)
{
  if (Diagonal==FALSE) {diag(NET1)<-0}
  #use sna::geodist from sna
  NN1<-dim(NET1)[1]
  GDIST1 <- sna::geodist(NET1)$gdist
  reach <- 1*is.finite(GDIST1)
  if (Diagonal==FALSE) {diag(GDIST1)<-NA}
  OUTPUT1<-matrix(NA,NN1,length(Beta))
  kn <- 1
  for (Beta.k in Beta)
  {
    if (Beta.k == 1)
    {
      OUTPUT1[, kn] = rowSums(xIsValid(GDIST1))
    }
    else
    {
      OUTPUT1[, kn] <- rowSums(reach * Beta.k ^ (GDIST1 - 1), na.rm = TRUE)
    }
    if (Normalize) {
      OUTPUT1[, kn] = OUTPUT1[, kn]/(NN1 - 1)
    }
    kn <- kn + 1
  }
  rownames(OUTPUT1)<-rownames(NET1)
