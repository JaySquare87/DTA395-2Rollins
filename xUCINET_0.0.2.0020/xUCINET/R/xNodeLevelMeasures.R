...
#' @param Diagonal Logical, whether the diagonal values should be ignored (default setting is FALSE, i.e., to ignore the diagonal values)
                                  ...
                                  Mode="OneMode")
{
  ...
  {
    if (Diagonal==FALSE) {diag(LIST1[[MATi]])<-NA}
                                ...
                                NrCategoriesIQV=1)
{
  ...
  if(Diagonal==FALSE) {diag(NET1)<-NA}
...
#'          }
#' @param Diagonal Whether to ignore the diagonal (default is to ignore diagonal).
                               ...
                               Mode="OneMode")
{
  ...
  if (Mode=="TwoMode")
  {
    ...
    {
      cat( '== == == == == == == == == == == == == == == == == == == == == == == == == == == == == ==', '\n',
           'WARNING: The network was indicated as being two-mode, yet it was requested that the diagonal would be removed.', '\n',
           '   This is not possible for a two-mode network. The diagonal was not removed.', '\n')
      Diagonal<-TRUE
    }
  }
...
#' @param Diagonal Whether to ignore the diagonal (default is to ignore the diagonal).
#' @param Exclude0 Whether to exclude values 0 from the analysis (default is FALSE).
#' @param Mode Whether NET1 is a one-mode ("OneMode") or two-mode ("TwoMode") network.
#' @return A matrix containing different measures (columns) for each node (row).
#' @export
...
#' @param Diagonal Whether to include the diagonal (default is to ignore the diagonal).
...
{
  if (xIsDirected(NET1)) {
    stop('Function not yet able to handle directed data')
  }
  ...
  {
    OUTPUT2 = vector(mode = "numeric",length = n)
    for (i in 1:n) {
      isalterof = NET1[,i] > 0
      temp = matrix(0,n,n)
      temp[,i] = NET1[,i]
      temp[i,] = NET1[i,]
      temp[isalterof,isalterof] = NET1[isalterof,isalterof]
      DEGREE<-rowSums(temp,na.rm=TRUE)
      NET1P<-temp/(DEGREE+(DEGREE==0))
      NET1D<- temp > 0
      DYADCONSTR <- ((NET1P+(NET1P%*%NET1P))^2)*NET1D
      OUTPUT2[i] <- rowSums(DYADCONSTR)[i]
      print(rowSums(DYADCONSTR))
    }
    OUTPUT1<-cbind(OUTPUT1,OUTPUT2)
